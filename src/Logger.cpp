...
Logger* Logger::init(const char *log_filename, const char *debug_filename) {
    if (!singleton_instance)   // Only allow one instance of class to be generated.
        singleton_instance = new Logger(log_filename, debug_filename);
    return singleton_instance;
}
...
Logger::Logger(const char *log_filename, const char *debug_filename) {
    ...
    else if ( (logFile=fopen(log_filename, "a+")) == nullptr)
    ...
    if (debug_filename == nullptr)
        ...
        if (log_filename != nullptr and strcmp(log_filename, debug_filename) == 0)
            debugFile = logFile;

        else if ( (debugFile=fopen(debug_filename, "w")) == nullptr)
            throw strerror(errno);
        else
            // Indicate this is a real file so that we can close it when needed.
...
Logger::~Logger() {
     ...
     * Close open files
     */
    if (logFile_REALFILE && logFile != nullptr)
        fclose(logFile);

    if (debugFile_REALFILE && debugFile != nullptr)
        fclose(debugFile);
}
...
void Logger::Print(const char *sev, const char *func_name, const char *msg, ...)
{
    ...
    printV(sev, logFile, nullptr, 0, func_name, msg, args);
    fflush(logFile);
                    ...
                    va_list args)
{
    ...
    if (filename != nullptr) {
        ...
        (fname = strrchr(filename, '/')) != nullptr ? fname++ : fname = filename;
